...
This method returns an `ITransport` implementation that interacts with the transport mechanism represented by the `scheme` in the uri.  This method will attempt to return a cached `ITransport` instance.  If none is found a new instance is requested using the `ITransportFactoryService.Get()` method.
