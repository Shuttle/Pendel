...
When you `Send` a *command* Shuttle needs to be able to determine the relevant transport.  The same applies to publishing an event.  Shuttle would need to know where to `Publish` the event.  When sending a command there should be 1, and exactly 1, transport that receives the command.  When publishing, however, there can be 0 to any number of subscribers.
